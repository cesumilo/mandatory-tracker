...
suspend fun getNextMatchInfo(context: Context): Pair<String, String>? = withContext(Dispatchers.IO) {
    try {
        ...
        if (nextMatch != null) {
            ...
            val notificationTime = if (oneHourBefore > now) oneHourBefore else if (matchTimeMs > now) matchTimeMs else null
            
            Pair(opponent, notificationTime?.let {
                val diff = it - now
