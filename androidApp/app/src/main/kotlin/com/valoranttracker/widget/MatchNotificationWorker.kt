...
class MatchNotificationWorker(
                ...
                when {
                    ...
                    matchTimeMs > now -> {
                        showImmediateNotification(
                            nextMatch.id,
                            nextMatch.event,
                            nextMatch.teams.find { it.id != teamId }?.name ?: "TBD",
                        )
                        prefs.edit().putString("scheduled_notification_match_id", nextMatch.id).apply()
                    }
                    else -> {
    ...
    private fun showImmediateNotification(
        ...
        val notification =
            ANotification.Builder(context, CHANNEL_ID)
    ...
    companion object {
        ...
        fun schedule(context: Context) {
            ...
            val request =
                PeriodicWorkRequestBuilder<MatchNotificationWorker>(
                    1,
                    TimeUnit.HOURS,
                    15,
                    TimeUnit.MINUTES,
...
class NotificationReceiver : BroadcastReceiver() {
    override fun onReceive(
        ...
        val notification =
            ANotification.Builder(context, MatchNotificationWorker.CHANNEL_ID)
                .setSmallIcon(R.drawable.mandatory_logo)
